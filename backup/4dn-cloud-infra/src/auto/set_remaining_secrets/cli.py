...
from ..init_custom_dir.utils import (obfuscate)
...
def establish_custom_aws_creds(custom_dir: str = None):
    try:
        custom_dir = get_custom_dir(custom_dir)
        custom_aws_creds_dir = get_custom_aws_creds_dir(custom_dir)
        aws_credentials_file = os.path.join(custom_aws_creds_dir, "credentials")
        aws_config_file = os.path.join(custom_aws_creds_dir, "config")
...
def aws_validate_account(custom_dir: str = None) -> [str, str, str]:
    with establish_custom_aws_creds(custom_dir):
        ...
        return account_number, access_key, secret_key, region
    return None, None, None, None


def aws_find_secret_name(secret_name_pattern: str) -> str:
    with establish_custom_aws_creds(custom_dir):
...
def aws_get_secret_value(secret_name: str, secret_key_name: str) -> str:
    with establish_custom_aws_creds(custom_dir):
        secrets_manager = boto3.client('secretsmanager')
...
def aws_create_user_access_key(user_name: str) -> [str,str]:
    with establish_custom_aws_creds(custom_dir):
...
def aws_find_iam_user(iam_user_name_pattern: str):
    with establish_custom_aws_creds(custom_dir):
        iam = boto3.resource('iam')
        iam_users = iam.users.all()
        for iam_user in sorted(iam_users, key=lambda user: user.name):
            iam_user_name = iam_user.name
            if iam_user_name_pattern and not re.search(iam_user_name_pattern, iam_user_name):
                continue
            return iam_user_name
        return None


def aws_get_kms_keys(customer: bool):
    result_keys = []
    with establish_custom_aws_creds(custom_dir):
...
def get_federated_user_name():
    ...
    return aws_find_iam_user(federator_user_name_pattern)
...
def aws_update_secret_key_value(secret_name: str,
                                secret_key_name: str,
...
aws_account_number, aws_access_key, aws_secret_key, aws_region = aws_validate_account(custom_dir)
print(f"Your AWS access key: {aws_access_key}")
print(f"Your AWS access secret: {obfuscate(aws_secret_key)}")
if account_number != aws_account_number:
    custom_config_file = get_custom_config_file()
    print(f"WARNING: Account number from your config file ({account_number}) not the same AWS credentials account ({aws_account_number}).")
secrets_to_update["ACCOUNT_NUMBER"] = account_number

# Get the IAM "federator" user name.
iam_federator_user_name = get_federated_user_name()
...
key_id, key_secret = aws_create_user_access_key(iam_federator_user_name)
...
es_server = get_opensearch_endpoint(aws_credentials_name)
...
rds_secret_name = aws_find_secret_name("C4DatastoreCgapSupertestRDSSecret")
print(f"RDS secret name is: {rds_secret_name}")
rds_hostname = aws_get_secret_value(rds_secret_name, "host")
print(f"RDS host name is: {rds_hostname}")
rds_password = aws_get_secret_value(rds_secret_name, "password")
...
s3_encrypt_key_id = aws_get_kms_keys(True)
...
if yes_or_no == "yes":
    for secret_key_name, secret_key_value in secrets_to_update.items():
        ...
        aws_update_secret_key_value(identity, secret_key_name, secret_key_value)
