...
#   See: AwsContext.establish_credentials()
#
# - ENCODED_IDENTITY
#   Get this from the identity, i.e. the GAC name,
#   e.g. C4DatastoreCgapSupertestApplicationConfiguration.
#   See: Names.application_configuration_secret(aws_credentials_name)
#
# - ENCODED_ES_SERVER
...
#   See: AwsFunctions.get_opensearch_endpoint()
...
#   ending in "RDSecret" in the AWS Secrets Manager. The "RDSSecret" value is from
#   RDS_SECRET_NAME_SUFFIX in C4Datastore in datastore.py; the "password" value is
...
#   See: AwsFunctions.get_customer_managed_kms_keys()
#
# - S3_AWS_ACCESS_KEY_ID, S3_AWS_SECRET_ACCESS_KEY
#   Get these by creating AWS security credentials for the "federated" IAM user.
#   See: AwsFunctions.create_user_access_key()
...
def main():
    ...
    if args.federated_user:
        ...
        #
        # TODO: The "ApplicationS3Federator" value here is from logical_id in ecs_s3_iam_user()
        # in iam.py. Alternatively, can get the federated user name from "C4IAMMainECSS3IAMUser"
        # in the AWS Outputs for the main stack, where the "C4IAMMainECSS3IAMUser" value is
        # from C4IAMExports.S3_IAM_USER, though really from the call in iam.py
        # to output_assumed_iam_role_or_user() in build_template().
        #
    ...
    if not federated_user_name:
        print(f"AWS federated user cannot be determined!")
    else:
